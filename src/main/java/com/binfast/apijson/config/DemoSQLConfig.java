...
//            return "8.0.11"; //TODO 改成你自己的 MySQL 或 PostgreSQL 数据库版本号 //MYSQL 8 和 7 使用的 JDBC 配置不一样
	...
	public String getDBUri() {
		...
		if (isMySQL()) {
			...
			// 以下是 MySQL 5.7 及以下
			return "jdbc:mysql://172.16.1.217:3306?serverTimezone=GMT%2B8&useUnicode=true&characterEncoding=UTF-8"; //TODO 改成你自己的，TiDB 可以当成 MySQL 使用，默认端口为 4000
		}
		if (isPostgreSQL()) {
	...
	public String getDBPassword() {
		...
		if (isMySQL()) {
			return "root123456";  //TODO 改成你自己的，TiDB 可以当成 MySQL 使用， 默认密码为空字符串 ""
		}
		if (isPostgreSQL()) {
